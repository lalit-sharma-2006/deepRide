  ...
  try {
    ...
    if (response.data.status === "OK") {
      ...
      if (!element.distance || !element.duration) {
        throw new Error("Invalid distance or duration data from Google Maps");
      }

      return element;
    } else {
      console.error("Distance Matrix API Error:", response.data.status, response.data.error_message);
      throw new Error(`Unable to fetch distance and time: ${response.data.error_message || response.data.status}`);
    }
  } catch (err) {
