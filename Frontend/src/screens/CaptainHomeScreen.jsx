...
function CaptainHomeScreen() {
  ...
  const [mapLocation, setMapLocation] = useState(
    `https://www.google.com/maps?q=28.7041,77.1025&output=embed`
  );
  ...
  // Initialize newRide with validation
  const getInitialRide = () => {
    try {
      const stored = localStorage.getItem("rideDetails");
      if (stored) {
        const ride = JSON.parse(stored);
        // Only use stored ride if it has valid ride ID
        if (ride._id && ride._id !== defaultRideData._id) {
          return ride;
        }
      }
    } catch (e) {
      Console.log("Error parsing stored ride:", e);
    }
    return defaultRideData;
  };

  const [newRide, setNewRide] = useState(getInitialRide());
  ...
  const [showNewRidePanel, setShowNewRidePanel] = useState(
    (() => {
      try {
        return JSON.parse(localStorage.getItem("showPanel")) || false;
      } catch (e) {
        return false;
      }
    })()
  ...
  const [showBtn, setShowBtn] = useState(
    (() => {
      try {
        return JSON.parse(localStorage.getItem("showBtn")) || "accept";
      } catch (e) {
        return "accept";
      }
    })()
  ...
  const acceptRide = async () => {
    try {
      if (!token) {
        showAlert('Error', 'Not authenticated. Please login again.', 'failure');
        return;
      }
      if (newRide._id != "" && newRide._id !== defaultRideData._id) {
        setLoading(true);
        ...
        // Join ride room to receive/send socket events for this ride
      ...
      setLoading(false);
      showAlert('Some error occured', error.response?.data?.message || error.message, 'failure');
  ...
  const verifyOTP = async () => {
    try {
      ...
      if (newRide._id != "" && newRide._id !== defaultRideData._id && otp.length == 6) {
        setLoading(true);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Send location update with ride ID if ride is accepted
          socket.emit("update-location-captain", {
            userId: captain._id,
            rideId: newRide._id && newRide._id !== defaultRideData._id ? newRide._id : null,
            location: {
              ltd: position.coords.latitude,
              lng: position.coords.longitude,
            },
  ...
  useEffect(() => {
    const storedCaptainId = localStorage.getItem("currentCaptainId");
    const currentCaptainId = captain._id;

    // If captain changed, clear all ride and panel data
    if (storedCaptainId !== currentCaptainId) {
      localStorage.setItem("currentCaptainId", currentCaptainId || "");
      localStorage.removeItem("rideDetails");
      localStorage.removeItem("showPanel");
